...
class CreateVenueType extends StatefulWidget {
  const CreateVenueType({super.key});
  @override
  State<CreateVenueType> createState() => _CreateVenueTypeState();
}

class _CreateVenueTypeState extends State<CreateVenueType> {
  ...
  Future<void> createVenue() async {
    try {
      ...
      final Map<String, dynamic> requestBody = {
        "name": Get.find<PostController>().venueTypeCtrl.text,
      };

      logger.i(requestBody);

      final response = await dio.post(
        AppUrl.getVenue, // Replace with your API endpoint
        data: requestBody,
        options: Options(
          headers: {
            'Authorization': token,
      ...
      if (response.statusCode == 200) {
        ...
        Get.back();
        Utils.flushBarSuccessMessage('Venue created successfully!', context);
        stopProgress();
        Get.find<PostController>().venueTypeCtrl.clear();
              ...
              child: Column(
                ...
                children: [
                  ...
                  EditProfileTextField(
                    controller: Get.find<PostController>().venueTypeCtrl,
                    hintText: '',
